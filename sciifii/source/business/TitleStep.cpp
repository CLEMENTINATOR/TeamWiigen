	...
	else /*  Si tid */
	{
		...
		if (_action != ti_Uninstall && _action != ti_Extract)
		{
			...
			wad << Config::WorkingDirectory() << "/"
					<< Title::GetWadFormatedName(_id, _revision);
			_file = wad.str();
			...
			if (!File::Exists(_file))
			{
				if (Config::HasNetwork())
				{
					...
					stringstream down;
					down << "Downloading title " << hex << setfill('0')
							<< setw(16) << _id << dec;
					if (_revision != 0)
					...
					ios.LoadFromNusServer(_id, _revision,
				...
				{
					cout
							<< "You arent connected to the network and some wads are missing."
							<< endl << "Please refer to the readme.";
					return false;
				}
			}
		}
