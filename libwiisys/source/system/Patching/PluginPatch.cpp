...
PluginPatch::PluginPatch(const Buffer& plugin, const u32 offset,
		const u32 bssNewSize, const std::string &module) :
	Patch(module), _plugin(plugin), _offset(offset), _bssNewSize(bssNewSize),
			_commandHandles(), _newProgramSection(NULL), _replaceSection(0) {
}

PluginPatch::PluginPatch(const PluginPatch& patch) :
	Patch(patch), _plugin(patch._plugin), _offset(patch._offset), _bssNewSize(
			patch._bssNewSize), _commandHandles(patch._commandHandles),
			_newProgramSection(NULL), _replaceSection(patch._replaceSection) {
	if (patch._newProgramSection) {
		_newProgramSection = new Elf32_Phdr(*(patch._newProgramSection));
	}
}

PluginPatch::PluginPatch(const Buffer& plugin, Elf32_Phdr newHeader,
		const std::string &module, u32 replaceSection) :
	Patch(module), _plugin(plugin), _offset(0), _bssNewSize(0),
			_commandHandles(), _newProgramSection(NULL), _replaceSection(
					replaceSection) {
	_newProgramSection = new Elf32_Phdr(newHeader);
	_newProgramSection->p_filesz = _plugin.Length();
	_newProgramSection->p_memsz = _newProgramSection->p_filesz;
}
...
u32 PluginPatch::FindIOSVersionIndex(const Buffer& b) const {
	...
	for (u64 cnt = 0; cnt < (b.Length() - str.size()); cnt++)
		...
		if (strncmp(str.c_str(), buf + cnt, str.length()) == 0) {
			...
			while (buf[cnt] == ' ')
				cnt++;
			if (strncmp(AllowedModule.c_str(), buf + cnt,
					AllowedModule.length()) == 0)
...
u32 PluginPatch::FindPlugedSegment(u8* elf, u32 moduleToSkip) const {
	...
	for (u32 phIndex = 0; phIndex < header->e_phnum; phIndex++) {
		Elf32_Phdr* ph = (Elf32_Phdr*) (elf + header->e_phoff + phIndex
				* header->e_phentsize);
...
u32 PluginPatch::FindSegmentSize(u8* elf, u32 segmentIndex) const {
	Elf32_Ehdr* header = (Elf32_Ehdr*) elf;
	Elf32_Phdr* pheader = (Elf32_Phdr*) (elf + header->e_phoff + segmentIndex
			* header->e_phentsize);
...
void PluginPatch::Plug(u32 segmentIndex, u32 bssSegmentIndex, u8* source,
		u8* dest) const {
	...
	//copy of pht
	memcpy(dest + outPos, source + inHeader->e_phoff, inHeader->e_phnum
			* inHeader->e_phentsize);
	...
	for (Elf32_Half phIndex = 0; phIndex < inHeader->e_phnum; phIndex++) {
		Elf32_Phdr* pHeader = (Elf32_Phdr*) (dest + outHeader->e_phoff
				+ phIndex * outHeader->e_phentsize);
		...
		else if (pHeader->p_type == PT_LOAD && pHeader->p_offset
				== inHeader->e_phoff) {
...
void PluginPatch::Plug(u32 segmentIndex, u8* source, u8* dest) const {
	...
	if (newSegment) {
		...
		segmentIndex = inHeader->e_phnum;
	}

	//copy of each sections, except the pht again
	for (Elf32_Half phIndex = 0; phIndex < outHeader->e_phnum; phIndex++) {
		Elf32_Phdr* pHeader = (Elf32_Phdr*) (dest + outHeader->e_phoff
				+ phIndex * outHeader->e_phentsize);
...
u32 PluginPatch::Patching(TitleEventArgs &processControl) const {
	//apply cmd handles
	for (vector<SimplePatch>::const_iterator ite = _commandHandles.begin(); ite
			!= _commandHandles.end(); ite++)
	...
	processControl.buffer.Append(destElf,
			TITLE_ROUND_UP(newElfLength + armLength, 64));
