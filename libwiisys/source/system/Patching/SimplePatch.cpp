...
SimplePatch::SimplePatch(const u8* pattern, const u8* patch, const u32 length,
...
{
  u8 oldCode[] =
    { 0xB5, 0x00, 0x4B, 0x09, 0x22, 0x01, 0x80, 0x1A, 0x22, 0xF0 };
  u8 newCode[] =
    { 0xB5, 0x00, 0x4B, 0x09, 0x22, 0x00, 0x80, 0x1A, 0x22, 0xF0 };
...
{
  u8 oldCode[] = { 0xFC, 0x0F, 0xB5, 0x30, 0x1C, 0x14, 0x1C, 0x1D, 0x4B,
                   0x0E, 0x68, 0x9B, 0x2B, 0x00, 0xD0, 0x03, 0x29, 0x00, 0xDB, 0x01,
                   0x29, 0x0F, 0xDD, 0x01 };
  u8 newCode[] = { 0xFC, 0x0F, 0xB5, 0x30, 0x1C, 0x14, 0x1C, 0x1D, 0x4B,
...
{
  u8 oldCode[] = { 0x00, 0x01, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46,
                   0x0A, 0x00, 0x00 };
  u8 newCode[] = { 0x7E, 0xD4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46,
                   0x0A, 0x00, 0x00 };
...
{
  if (s == "ES_HashCheck_Old")
    return ES_HashCheck_Old();
  else if (s == "ES_HashCheck_New")
    return ES_HashCheck_New();
  else if (s == "ES_Identify")
    return ES_Identify();
  else if (s == "ES_OpenTitleContent1")
    return ES_OpenTitleContent1();
  else if (s == "ES_OpenTitleContent2")
    return ES_OpenTitleContent2();
  else if (s == "ES_ReadContent")
    return ES_ReadContent();
  else if (s == "ES_CloseContent")
    return ES_CloseContent();
  else if (s == "ES_SetUIDCheck")
    return ES_SetUIDCheck();
  else if (s == "ES_TitleVersionCheck")
    return ES_TitleVersionCheck();
  else if (s == "ES_TitleDeleteCheck")
    return ES_TitleDeleteCheck();
  else if (s == "ES_MEM2Protection")
    return ES_MEM2Protection();
  else if (s == "FFS_PermsCheck")
    return FFS_PermsCheck();
  else if (s == "DIP_UnencryptedLimit")
    return DIP_UnencryptedLimit();
  else if (s == "DIP_EnableDvdVideo")
    return DIP_EnableDvdVideo();
